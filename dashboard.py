...
class Promethus_Grafana:
    ...
    def promethus_push_to(self, job_name):
        ...
        push_to_gateway(PUSH_GATEWAY_URL, job=job_name,
    ...
    def generate_link_public(self, tag):
        try:
            ...
            requests.post(url, headers=headers, json={
                        "isEnabled": True}, auth=HTTPBasicAuth('admin', 'admin123@'))
    ...
    def collect_gpu_metrics(self, registry):
        ...
        if os_name == "Darwin":
            ...
            gpu_memory_total = Gauge('gpu_0_memory_total_mb', 'GPU 0 Total Memory in MB', registry=registry)
            gpu_memory_total.set(vram_total)
            # Các thông tin khác không có sẵn nên đặt giá trị mặc định (0)
            gpu_load = Gauge('gpu_0_load_percent', 'GPU 0 Load Percentage', registry=registry)
            gpu_load.set(0)
            gpu_memory_used = Gauge('gpu_0_memory_used_mb', 'GPU 0 Memory Used in MB', registry=registry)
            gpu_memory_used.set(0)
            gpu_temperature = Gauge('gpu_0_temperature_celsius', 'GPU 0 Temperature in Celsius', registry=registry)
            ...
            if gpus:
                for i, gpu in enumerate(gpus):
                    gpu_load = Gauge(f'gpu_{i}_load_percent', f'GPU {i} Load Percentage', registry=registry)
                    gpu_load.set(gpu.load * 100)
                    
                    gpu_memory_used = Gauge(f'gpu_{i}_memory_used_mb', f'GPU {i} Memory Used in MB', registry=registry)
                    gpu_memory_used.set(gpu.memoryUsed)
                    
                    gpu_memory_total = Gauge(f'gpu_{i}_memory_total_mb', f'GPU {i} Total Memory in MB', registry=registry)
                    gpu_memory_total.set(gpu.memoryTotal)
                    
                    gpu_temperature = Gauge(f'gpu_{i}_temperature_celsius', f'GPU {i} Temperature in Celsius', registry=registry)
    ...
    def collect_system_metrics(self, job_name):
        ...
        cpu_cores = Gauge('cpu_cores', 'Number of Physical CPU Cores', registry=registry)
        cpu_cores.set(psutil.cpu_count(logical=False))

        cpu_threads = Gauge('cpu_threads', 'Number of Logical CPU Threads', registry=registry)
        ...
        ram_percent = Gauge('ram_usage_percent', 'RAM Usage Percentage', registry=registry)
        ...
        disk_percent = Gauge('disk_usage_percent', 'Disk Usage Percentage', registry=registry)
        ...
        print("Đã push metrics info compute tới", f"{PUSH_GATEWAY_URL}/metrics/job/{job_name}")
...
def genarate_dashboard(job_name):
    ...
    promethus_grafana.create_dashboard(
        job_name, "Training Job", job_name, [job_name])
